  ...
  return (
    <div className="p-6" data-testid="sensor-management-container">
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Sensor Management</h1>
          <p className="text-gray-600 dark:text-gray-300 mt-1">Manage door sensors - Create, Edit, Delete & Test</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sensors.map((sensor) => (
          <div
            key={sensor.id}
            className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5 border-l-4 border-blue-600"
            data-testid={`sensor-mgmt-card-${sensor.id}`}
          >
            <div className="flex justify-between items-start mb-3">
              <div className="flex-1">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{sensor.name}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">{sensor.location}</p>
                {sensor.esp8266_mac && (
                  <p className="text-xs text-gray-500 dark:text-gray-400 font-mono mt-1">MAC: {sensor.esp8266_mac}</p>
                )}
              </div>
              <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                sensor.is_enabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800 dark:text-gray-100'
              }`}>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor="is_enabled" className="text-sm font-medium text-gray-700 dark:text-gray-200">
